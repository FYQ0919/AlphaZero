...
def MCTS(model, observation, num_simulations=10, minimax=True):
  ...
  policy, value = model(torch.tensor(observation).float())
...
def train(batch_size=128):
  if(len(memory) >= batch_size):
    for i in range(10):
      states, actions, values = memory.sample(batch_size)
      pi, v = net(states)

      policy_loss = -(actions * torch.log(pi)).sum(dim=1) 
...
if __name__ == '__main__':
  ...
  for epi in range(100):
    ...
    score = 0
    while not done:
      policy, value, _ = MCTS(net, state, 10)
      #print(policy)
      ...
      memory.store(state,policy,value)
      #env.render()	
